    ...
    function initialize() 
    {
        ...
        function encontrarDireccion(latitud,longitud)
        {
          ...
          success(function (data){
              ...
              else if( (typeof(street_number) != "undefined") || (typeof(sublocality) != "undefined") || (street_number.length >=1) || (sublocality.length >=1) )
              {
...
.controller('DenunciarController2',function($scope, $http, $state, $cordovaCamera,$cordovaFileTransfer,SrcImagenService,DireccionService, userService, ImagenService){
  ...
  $scope.visibilidadDependencia = false;
  
  $scope.direccion = {};
  $scope.data = {};
  $scope.imgURI = SrcImagenService.imagen.getProperty();
  
  $http.get('http://www.denunciaciudadana.esy.es/index.php/reportes/subcategorias/')
  .success(function(data){
    console.log("subcategorias: "+ data.response);
    $scope.subcategorias =  data.response;
  }); 

$http.get('http://www.denunciaciudadana.esy.es/index.php/reportes/categorias/')
  .success(function(data){
    console.log("categorias: "+data.response);
    $scope.detalles =  data.response;
  }); 

$scope.cambio =function(){

$scope.visibilidadDependencia = true;

  $http.get('http://www.denunciaciudadana.esy.es/index.php/reportes/categoria_sub/'+$scope.data.subcategoria)
  .success(function(data){
    console.log(data.response[0].id_categoria);
      $scope.data.categoria = data.response[0].id_categoria;    
  }); 
